...
// bool utils::setInputStream(const utils::InputStream& source, const std::string& imagePath, const std::string& videoPath, const std::string& rtspURL, const int& cameraID,
//                            cv::VideoCapture& capture, int& totalBatches, int& delayTime, utils::InitParameter& param)
...
// bool utils::setInputStream(const utils::InputStream& source, const std::string& imagePath, const std::string& videoPath, const std::string& rtspURL1, const std::string& rtspURL2, const int& cameraID,
//                            cv::VideoCapture& capture1, cv::VideoCapture& capture2, int& totalBatches, int& delayTime, utils::InitParameter& param)
// {
//     int total_frames = 0;
//     std::string img_format;
//     switch (source)
...
//                 sample::gLogWarning << "+-----------------------------------------------------------+" << std::endl;
//             }
//             capture1.open(imagePath); // cv::CAP_IMAGES : OpenCV Image Sequence (e.g. img_%02d.jpg)
//             param.batch_size = 1;
//             total_frames = 1;
//             totalBatches = 1;
//             delayTime = 0;
//             break;
//         case utils::InputStream::VIDEO:
//             capture1.open(videoPath);
//             total_frames = capture1.get(cv::CAP_PROP_FRAME_COUNT);
//             totalBatches = (total_frames % param.batch_size == 0) ?
//                            (total_frames / param.batch_size) : (total_frames / param.batch_size + 1);
//             break;
//         case utils::InputStream::CAMERA:
//             capture1.open(cameraID);
//             total_frames = INT_MAX;
//             totalBatches = INT_MAX;
//             break;
//         case utils::InputStream::RTSP:
//             capture1.open(rtspURL1);
//             capture2.open(rtspURL2);
//             total_frames = INT_MAX;
//             totalBatches = INT_MAX;
//             break;
//         default:
//             break;
//     }
//     param.src_h = capture1.get(cv::CAP_PROP_FRAME_HEIGHT);
//     param.src_w = capture1.get(cv::CAP_PROP_FRAME_WIDTH);

//     return capture1.isOpened() && capture2.isOpened();
// }
...
void utils::showClassifications(const std::vector<std::pair<int, float>>& classifications, const std::vector<std::string>& class_names, const int& delayTime, std::vector<cv::Mat>& imgsBatch)
{
    for (size_t i = 0; i < imgsBatch.size(); ++i)
    {
        int class_id = classifications[i].first;
        float confidence = classifications[i].second;
        std::string label = class_names[class_id] + " (" + std::to_string(confidence) + ")";
        cv::putText(imgsBatch[i], label, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
        cv::imshow("Classification", imgsBatch[i]);
        cv::waitKey(delayTime);
    }
}


void utils::saveClassifications(const std::vector<std::pair<int, float>>& classifications, const std::vector<std::string>& class_names, const std::string& save_path, std::vector<cv::Mat>& imgsBatch, const int& batch_size, const int& batchi)
{
    for (size_t i = 0; i < imgsBatch.size(); ++i)
    {
        int class_id = classifications[i].first;
        ...
        std::string label = class_names[class_id] + " (" + std::to_string(confidence) + ")";
        cv::putText(imgsBatch[i], label, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
        int imgi = batchi * batch_size + i;
        cv::imwrite(save_path + "_" + std::to_string(imgi) + ".jpg", imgsBatch[i]);
    }
}
	...
	const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch)
{
	...
	for (size_t bi = 0; bi < imgsBatch.size(); bi++)
	{
        ...
        if (classNames.size() == 83) 
        {
          color = Colors::color83[box.label];
        }
        if (classNames.size() == 5) 
        {
          color = Colors::color5[box.label];
        }
	...
	const std::string& savePath, std::vector<cv::Mat>& imgsBatch, const int& batchSize, const int& batchi)
{
    ...
    cv::imwrite(savePath + "_" + std::to_string(imgi) + "_s" + ".jpg", imgsBatch[bi]);
		if (!objectss.empty())
		{
			for (auto& box : objectss[bi])
			{

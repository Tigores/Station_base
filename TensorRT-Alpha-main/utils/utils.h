...
namespace utils 
{
    namespace dataSets
    {
        ...
        const std::vector<std::string> fire_smoke = {"fire", "smoke"};

        const std::vector<std::string> cell_phone = {"phone"};

        const std::vector<std::string> smoke = {"smoking"};

        const std::vector<std::string> spray = {"spray","no_spray"};

        const std::vector<std::string> datasets_merge = {
            "fire", "smoke","smoking",
            "person", "bicycle", "car", "motorcycle", "airplane", "bus", "train", "truck", "boat", "traffic light",
            "fire hydrant", "stop sign", "parking meter", "bench", "bird", "cat", "dog", "horse", "sheep", "cow",
            "elephant", "bear", "zebra", "giraffe", "backpack", "umbrella", "handbag", "tie", "suitcase", "frisbee",
        ...
        const std::vector<std::string> weather_5 = {
            "cloudy", "foggy","rainy",
            "snowy","sunny"
        };
    }
    namespace Colors
    {
        ...
        const std::vector<cv::Scalar> color3{
            cv::Scalar(148, 80, 189),cv::Scalar(105, 52, 238),cv::Scalar(0, 52, 138)
        };
        ...
        const std::vector<cv::Scalar> color83{ 
            cv::Scalar(128, 77, 207),cv::Scalar(65, 32, 208),cv::Scalar(0, 224, 45),cv::Scalar(3, 141, 219),cv::Scalar(80, 239, 253),cv::Scalar(239, 184, 12),
            cv::Scalar(7, 144, 145),cv::Scalar(161, 88, 57),cv::Scalar(0, 166, 46),cv::Scalar(218, 113, 53),cv::Scalar(193, 33, 128),cv::Scalar(190, 94, 113),
            cv::Scalar(113, 123, 232),cv::Scalar(69, 205, 80),cv::Scalar(18, 170, 49),cv::Scalar(89, 51, 241),cv::Scalar(153, 191, 154),cv::Scalar(27, 26, 69),
            cv::Scalar(20, 186, 194),cv::Scalar(210, 202, 167),cv::Scalar(196, 113, 204),cv::Scalar(9, 81, 88),cv::Scalar(191, 162, 67),cv::Scalar(227, 73, 120),
            ...
            cv::Scalar(131, 254, 90),cv::Scalar(7, 221, 102),cv::Scalar(19, 191, 184),cv::Scalar(98, 126, 199),cv::Scalar(210, 61, 56),cv::Scalar(252, 86, 59),
            cv::Scalar(102, 195, 55),cv::Scalar(160, 26, 91),cv::Scalar(60, 94, 66),cv::Scalar(204, 169, 193),cv::Scalar(126, 4, 181),cv::Scalar(229, 209, 196),
            cv::Scalar(195, 170, 186),cv::Scalar(155, 207, 148),cv::Scalar(148, 80, 189),cv::Scalar(105, 52, 238),cv::Scalar(0, 52, 138)
        };
        const std::vector<cv::Scalar> color5{ 
    ...
    enum class InputStream { IMAGE, VIDEO, CAMERA, RTSP };
    ...
    // void showClassifications(const std::vector<int>& class_ids, const std::vector<std::string>& class_names, const int& delayTime, std::vector<cv::Mat>& imgsBatch);
    void showClassifications(const std::vector<std::pair<int, float>>& classifications, const std::vector<std::string>& class_names, const int& delayTime, std::vector<cv::Mat>& imgsBatch);
    void saveClassifications(const std::vector<std::pair<int, float>>& classifications, const std::vector<std::string>& class_names, const std::string& save_path, std::vector<cv::Mat>& imgsBatch, const int& batch_size, const int& batchi);
    ...
    bool setInputStream(const InputStream& source, const std::string& imagePath, const std::string& videoPath, const std::string& rtspURL, const int& cameraID,
                    cv::VideoCapture& capture, int& totalBatches, int& delayTime, InitParameter& param);
    // bool setInputStream(const InputStream& source, const std::string& imagePath, const std::string& videoPath, const std::string& rtspURL1, const std::string& rtspURL2, const int& cameraID,
    //                 cv::VideoCapture& capture1, cv::VideoCapture& capture2, int& totalBatches, int& delayTime, InitParameter& param);

...
app_run = "/home/jetson/dxh/TensorRT-Alpha-main/yolov8v1.0q/build/app_yolov8"
rtsps = [
...
def start_task(command, task_p, stop_p, id, task_flag):
    ...
    if task_p is None or stop_p is not None:  # 只有在需要时才启动新进程
        task_p = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        all_processes.append(task_p)  # 将新启动的进程加入全局进程列表
        threading.Thread(target=monitor_task_output, args=(task_p, task_flag, id), daemon=True).start()
...
def monitor_task_output(process, task_flag, id):
    try:
        for line in process.stdout:
            if "smoking detected" in line:
                print(f"Smoke detected for {id}")
                # 添加抽烟检测逻辑，例如发送警报
                idx = int(id)
                dalist = [2, idx, 'unknown']
                mess = encode(getdata(dalist, PW, MN))
                x = connect_server(init_socket(),HOST,PORT,mess)
            if "fire detected" in line:
                print(f"Fire detected for {id}")
                # 添加火灾检测逻辑，例如发送警报
                idx = int(id)
                dalist = [3, idx, 'unknown']
                mess = encode(getdata(dalist, PW, MN))
                x = connect_server(init_socket(),HOST,PORT,mess)
    except Exception as e:
        print(f"Error while monitoring task output for {id}: {e}")
    finally:
